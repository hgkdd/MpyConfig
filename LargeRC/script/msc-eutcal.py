...
cdict = {"autosave_filename": 'msc-autosave.p',
         ...
         "descriptions": ['EUT'],
         "measure_parameters": [{'dotfile': 'largeMSC_EUTcalib.dot',
                                 'delay': 1,
                                 'freqs': None,
                                 'SGLevel': -20,
                                 'leveling': None,
...
if __name__ == '__main__':
    ...
    if not msc:
        ...
        for _i,_des in enumerate(cdict['descriptions']):
            ...
            domeas=True
            doeval=True
            if msc.rawData_EUTCal.has_key(_des):
                domeas=False
                doeval=False
                msg = """"
                ...
                if answer == but.index('Break'):
                    ...
                    continue
            if domeas:        
                msc.Measure_EUTCal(**mp)
                pickle.dump(msc, file(cdict["after_measurement_pickle_file"]%_des, 'wb'),2)
            if doeval:
                msc.OutputRawData_EUTCal(fname=cdict["rawdata_output_filename"]%_des)
                msc.Evaluate_EUTCal(description=_des)
                msc.CalculateLoading_EUTCal(empty_cal=mp['calibration'], eut_cal=_des, freqs=mp['freqs'])
        msc.OutputProcessedData_EUTCal(fname=cdict["processeddata_output_filename"]%("_".join(descriptions)))
    else:
