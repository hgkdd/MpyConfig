...
cdict = {"autosave_filename": 'msc-autosave.p',
         "pickle_output_filename": 'msc-autocorrelation.p',
         "pickle_input_filename": None,
         "rawdata_output_filename": 'out_raw_autocorrelation-%s.dat',
         "processeddata_output_filename": 'out_processed_autotocorrelation-%s.dat',
         "log_filename": 'msc.log',
         "logger": ['stdlogger'],
         "minimal_autosave_interval": 3600,
         "descriptions": ['empty'],
         "measure_parameters": [{'dotfile': 'largeMSC-auto.dot',
                                 'delay': 1,
                                 'freqs': None,
                                 'SGLevel': -20,
                                 'leveling': None,
...
if __name__ == '__main__':
    ...
    if not msc:
        ...
        for _i,_des in enumerate(cdict['descriptions']):
            ...
            domeas=True
            doeval=True
            if msc.rawData_AutoCorr.has_key(_des):
                domeas=False
                doeval=False
                msg = """"
                ...
                if answer == but.index('Break'):
                    ...
                    continue
            if domeas:        
                msc.Measure_Autocorrelation(**mp)
            if doeval:
                msc.OutputRawData_AutoCorr(fname=cdict["rawdata_output_filename"]%_des)
                msc.Evaluate_AutoCorr(**ep)
        msc.OutputProcessedData_AutoCorr(fname=cdict["processeddata_output_filename"]%("_".join(descriptions)))
    else:
